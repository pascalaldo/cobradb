...
def get_or_create(session, query_class, do_commit=True, **kwargs):
...
def load_tsv(filename, required_column_num=None):
    ...
    with open(filename, "r") as f:
        ...
        rows = [
            [check_none(x.strip()) for x in line.split("\t")]
    ...
    if required_column_num is not None:
        ...
        def check_row(row):
            if len(row) != required_column_num:
                logging.warning(
                    "Line in {} should have {} columns, but found {}: {}".format(
                        filename, required_column_num, len(row), row
                    )
                )
                return None
            return row
...
def get_or_create_data_source(session, bigg_id):
    ...
    """
    data_source_db = (
        session.query(DataSource).filter(DataSource.bigg_id == bigg_id).first()
    )
...
def scrub_name(the_name):
    ...
    if the_name is None:
        return None
    the_name = (
        the_name.replace("_SPACE_SPACE_", " ")
        .replace("_SPACE_", " ")
        .replace("_COLON_", ":")
        .replace("_COMMA_", ",")
    )
