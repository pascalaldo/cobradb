...
def test_genbank_files():
    return [
        (("ncbi_accession", "core"), join(test_data_dir, "core.gb")),
...
def test_model_files():
    return [
        {
            "path": join(test_data_dir, "ecoli_core_model.xml"),
...
def test_db(request, test_db_create):
    ...
    engine = create_engine(
        "postgresql://%s:%s@%s/%s"
        % (user, settings.postgres_password, settings.postgres_host, test_db)
    )
...
def load_genomes(test_db, test_genbank_files, session):
    ...
    settings.gene_reaction_rule_prefs = join(
        test_data_dir, "gene-reaction-rule-prefs.txt"
    )
